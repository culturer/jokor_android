...
import com.jokor.base.util.base.ServiceUtil;
...
import static com.jokor.base.base.Urls.CODE_ADD_USERLIST;
import static com.jokor.base.base.Urls.CODE_GET_APPLIES;
...
//                .override(300,300)//指定图片的尺寸
    ...
    public void openP2PSession(FriendsBean friendsBean,Context context){
        ...
        Session session = sessionBox.query().equal(Session_.msgFrom,MSG_FROM_FRIEND).equal(Session_.Belong,Datas.getUserInfo().getId()).equal(Session_.Id,friendsBean.getFriend().getId()).build().findUnique();
        if ( session == null ){
            session = new Session() ;
    ...
    public void openGroupSession(GroupBean groupBean, Context context){
        ...
        //2.如果没有就创建后打开
        Box<Session> sessionBox =  DBManager.getInstance().getSessionBox();
        Session session = sessionBox.query().equal(Session_.msgFrom,MSG_FROM_GROUP).equal(Session_.Belong,Datas.getUserInfo().getId()).equal(Session_.Id,groupBean.getId()).build().findUnique();
        if ( session == null ){
            session = new Session() ;
